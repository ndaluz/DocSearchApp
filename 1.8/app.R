...
highlight_keyword <- function(text, keyword, ignore_case = FALSE) {
  ...
  highlighted_text <- str_replace_all(text, keyword, paste0("<span style='color:red; font-weight:bold;'>", "\\0", "</span>"))
...
keywordsearch <- function(x, keyword, path = FALSE,
  ...
  if (any(line_nums == 0)) {
    ...
    if (!is.null(keyword) && !is.null(text_out$line_text)) {
      text_out$line_text <- Map(highlight_keyword, text_out$line_text, keyword, ignore_case)
    }
